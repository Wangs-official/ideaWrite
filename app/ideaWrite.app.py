...
@app.route('/')
def index_page(LTime = time.strftime("%Y/%m/%d %H:%M", time.localtime(json.loads(requests.get(f'{api_url}/start').text)['LastLogin'])),NovelInfo='0'):
    if not os.path.exists("install.lock"):
        ...
        if ni_res.status_code == 200:
            ...
            return render_template('error.html' , pth = '/get/novel/',Error_info = ni_res.text , VersionCode = versioncode,statuscode = ni_res.status_code)
        ...
        if idea_res.status_code == 200:
            ...
            return render_template('error.html' , pth = '/get/idea/',Error_info = idea_res.text , VersionCode = versioncode,statuscode = idea_res.status_code)
        db_res = requests.get(f'{api_url}/get/dashboard')
        if db_res.status_code == 200:
            db = json.loads(db_res.text)
        elif db_res.status_code == 500:
            return render_template('error.html' ,pth = '/get/dashboard/', Error_info = db_res.text , VersionCode = versioncode,statuscode = db_res.status_code)
        return render_template('main.html', versionCode=versioncode,lastlogin=LTime,NovelInfo=NovelInfo,IdeaInfo=ideainfo,db=db)
